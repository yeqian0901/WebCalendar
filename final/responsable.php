...
if (array_key_exists('ajouterMatiere', $_POST)) {
    if (isset($_POST["ajouterMatiere"]) && $_POST["ajouterMatiere"] != "")
        array_push($listeMatiere, $_POST["ajouterMatiere"]);
}

if (array_key_exists('ajouterEnseignant', $_POST)) {
    if (isset($_POST["ajouterEnseignant"]) && $_POST["ajouterEnseignant"] != "")
        array_push($listeEnseignant, $_POST["ajouterEnseignant"]);
}

if (array_key_exists('ajouterSalle', $_POST)) {
    if (isset($_POST["ajouterSalle"]) && $_POST["ajouterSalle"] != "")
        array_push($listeSalle, $_POST["ajouterSalle"]);
}
...
<body>
    ...
    <form id="FormulaireAjouterListe" method="post">
        <input id="ajouterMatiere" name="ajouterMatiere" placeholder="ajouter une matière">
        <button type="submit" id="buttonAjouterMatiere" class="border">+</button>
        </br>

        <input id="ajouterEnseignant" name="ajouterEnseignant" placeholder="ajouter un enseigant">
        <button type="submit" id="buttonAjouterEnseignant" class="border">+</button>
        </br>

        <input id="ajouterSalle" name="ajouterSalle" placeholder="ajouter une salle">
        <button type="submit" id="buttonAjouterSalle" class="border">+</button>
        </br>
        </br>
    </form>


    <form id="formulaire" method="post" action="responsable.php" style="display:none">
        <p>
...
$json_data1 = json_encode($listeMatiere, JSON_PRETTY_PRINT);
file_put_contents("listeMatiere.json", $json_data1);

$json_data2 = json_encode($listeEnseignant, JSON_PRETTY_PRINT);
file_put_contents("listeEnseignant.json", $json_data2);

$json_data3 = json_encode($listeSalle, JSON_PRETTY_PRINT);
file_put_contents("listeSalle.json", $json_data3);
